...
- `just setup` — installs deps and runs DB migrations; auto-provisions the
...
The script installs inside the container: `gcc/gcc-c++`, `just`, `make`,
`python3`, `vips-devel` (required by `sharp`), Node.js 24 (NodeSource RPM),
and pnpm 10.17.1 (via corepack).
